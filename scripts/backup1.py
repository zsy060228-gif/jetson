...
#time3=time.time()
ret=Manager.Value(c_bool, False)
...
def update(que, video):
    global ret
    while 1:
        ret.value, img=video.read()
        lenth=que.qsize()
        if lenth>2:
...
if __name__ == "__main__":
    ...
    #video.set(6, cv2.VideoWriter_fourcc('M','J','P','G'))
    video.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
    video.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)

    isopen=False

    buff=video.get(cv2.CAP_PROP_BUFFERSIZE)
    print(buff)
    video.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    buff=video.get(cv2.CAP_PROP_BUFFERSIZE)
    print(buff)

    peocess1=Process(target=update, args=(que, video))
    process1.start()
    ...
    #while isopen:
        ...
        frame_1 = cv2.resize(frame,(0, 0), fx=0.25, fy=0.25, interpolation=cv2.INTER_NEAREST)
        ...
        # print(heigh) 
        #裁剪图像，预处理
        frame=frame[int(heigh*1/3):int(heigh*2/3),0:width]
        frame_cut = cv2.resize(frame,(0, 0), fx=0.25, fy=0.25, interpolation=cv2.INTER_NEAREST)
        #cv2.imshow('after cut', frame_cut)
        #cv2.imwrite("result/mask.jpg",frame)
        ...
        # print(Demoout.shape)
        #计算偏差角度
        #cv2.imwrite("result/output.jpg",out)
        vtherror=get_angle(out)
