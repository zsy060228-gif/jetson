...
crosswalkdelay=1.29 #小车识别到人行横道后延时这么长时间停车
updelay = 2.25         #识别到坡道后这么长时间再停
jiansu_delay1 = 3   #红线停止后再次启动时快速变为中速的过渡时间
jiansu_delay2 = 3   #红线停止后再次启动时中速变为慢速的过渡时间
godelay = 9        #红绿灯后延时这么久后停车
...
def callback(msg_old):
    ...
    global time2, time3, time4, time5, time6, slowedtime, redlightdelay, time7, updelay
    ...
    if speed_shift == 0:
        ...
        msg.linear.x = msg_old.linear.x * 0.9
        msg.angular.z = msg_old.angular.z * 0.9
    elif speed_shift == 2:
        msg.linear.x = msg_old.linear.x * 1.3
        msg.angular.z = msg_old.angular.z * 1.3
    elif speed_shift == 3:
        msg.linear.x = msg_old.linear.x * 2
        msg.angular.z = msg_old.angular.z * 2
    elif speed_shift == 4:
        msg.linear.x = msg_old.linear.x * 0.3
        msg.angular.z = -7
